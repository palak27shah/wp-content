<?php
	...
	abstract class WS_Form_Action {
		...
		public static function get_svg($action_id, $list_id, $label, $field_count, $record_count, $field_label = false, $record_label = false) {
			...
			$svg .= sprintf('<text fill="' . $color_default . '" class="wsf-template-title"><tspan x="' . (is_rtl() ? ($svg_width - 5) : 5) . '" y="16">%s</tspan></text>', htmlentities($label));
			...
			if($field_count !== false) {
				...
				$svg .= sprintf('<tspan x="' . (is_rtl() ? ($svg_width - 5) : 5) . '" y="%u" fill="' . $color_default . '">%s: <tspan class="wsf-template-stat-number" fill="' . $color_information . '">%u</tspan></tspan>', $ypos, $field_label, number_format($field_count));
			}
			...
			if($record_count !== false) {
				...
				$svg .= sprintf('<tspan x="' . (is_rtl() ? ($svg_width - 5) : 5) . '" y="%u" fill="' . $color_default . '">%s: <tspan class="wsf-template-stat-number" fill="' . $color_information . '">%u</tspan></tspan>', $ypos, $record_label, number_format($record_count));
			}
